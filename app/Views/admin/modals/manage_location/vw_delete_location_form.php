<form id="form_delete_location" action="<?= site_url('admin/manage/task/delete/location') ?>" method="post">
    <input type="hidden" name="_method" value="DELETE">
    <input type="hidden" name="id" value="<?= esc($location['id']) ?>">
    <div class="text-center">
        <p>Apakah Anda yakin ingin menghapus lokasi "<strong>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4">
        ...
        <button type="submit" id="btn_delete_lokasi" class="btn btn-danger d-flex align-items-center">
...
<script>
    $('#form_delete_location').validate($.extend({
        submitHandler: function (form, event) {
            event.preventDefault();
            let submit = $('#btn_delete_lokasi');
